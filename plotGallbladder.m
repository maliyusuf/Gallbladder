function plotGallbladder(PercentageOfLesion)
...
function [xx,yy,zz] = gall(xc,yc,zc,xr,yr,zr,n, cut)

theta = (-pi:2*pi/n:cut/2);
phi   = (-pi/2 : 2*pi/2/n : pi/2)';

% x = xr*cos(phi)*cos(theta);
% y = yr*cos(phi)*sin(theta);
% z = zr*sin(phi)*ones(1,n+1);

xx = xr * cos(phi) * sin(theta);
yy = yr * sin(phi) * sin(theta);
zz = zr * ones(size(yy,1),1) * cos (theta);
